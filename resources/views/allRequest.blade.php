                ...
                @foreach($books_taken as $book)
                        @if($book->permission == false)
                            <li class="collection-item avatar hover:bg-gray-200">
                                ...
                                <form action="{{route('accept.request', $book->id)}}" method="post" style="display: none" id="{{'order-accept-'.$book->id}}">
                                    @csrf
                                    @method('patch')
                                </form>
                                <a onclick="document.getElementById('order-accept-{{$book->id}}').submit();" class="hoverable btn-floating btn-large waves-effect waves-light green"><i class="material-icons">check</i></a>
                                ...
                                <a onclick="document.getElementById('order-delete-{{$book->id}}').submit();" class="hoverable btn-floating btn-large waves-effect waves-light white"><i class="material-icons red-text">close</i></a>
